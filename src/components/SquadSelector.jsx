...
export default function SquadSelector({ team, selection, onUpdateSelection, onClose }) {
  ...
  const handleFormationChange = (f) => {
    const newSlots = initSlots(f);
    const released = Object.values(slots).filter(Boolean);
    // released players go back to pool (pool is derived, no explicit state needed)
    update({ formation: f, slots: newSlots, bench: [] });
    // also release bench
  };

  const handleFormationChangeClean = (f) => {
    const newSlots = initSlots(f);
    update({ formation: f, slots: newSlots, bench: [] });
  };
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", background: "#0f0f0f", overflow: "hidden" }}>
        ...
        <div style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 20 }}>
            <div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontWeight: 900, fontSize: 16, letterSpacing: 1, color: "#e8e8e8" }}>
                SELECT TEAM — {team.name.toUpperCase()}
              </div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
              <span style={{ fontSize: 10, color: "#555", fontWeight: 700, letterSpacing: 1, fontFamily: "'Barlow Condensed', sans-serif" }}>FORMATION</span>
              {Object.keys(FORMATION_SLOTS).map(f => (
                <button key={f} onClick={() => handleFormationChangeClean(f)}
                  style={{
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <span style={{ fontSize: 12, color: "#555" }}>
              <span style={{ color: "#cc0000", fontWeight: 700, fontSize: 15 }}>{xiCount}</span>
              <span>/11 XI</span>
            </span>
            <button
              onClick={onClose}
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
          <div style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", padding: "16px 12px 8px" }}>
            <div style={{
              position: "relative",
              width: "min(460px, 100%)",
              aspectRatio: "2/3",
          ...
          {/* Bench */}
          <div
            style={{ background: "#111", borderTop: "1px solid #1a1a1a", padding: "8px 20px 12px" }}
            onDragOver={allowDrop}
